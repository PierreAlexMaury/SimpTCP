...
int makePDU (struct simptcp_socket * sockpdu, char * msg, unsigned char flags, int lenght_msg) {
	
	simptcp_set_sport (sockpdu->out_buffer, ntohs(sockpdu->local_simptcp.sin_port));	
	simptcp_set_dport (sockpdu->out_buffer, ntohs(sockpdu->remote_simptcp.sin_port));
	simptcp_set_flags  (sockpdu->out_buffer, flags);
	simptcp_set_seq_num   (sockpdu->out_buffer, (u_int16_t) sockpdu->next_seq_num); //peut-être conflit de type
	simptcp_set_ack_num   (sockpdu->out_buffer, (u_int16_t) sockpdu->next_ack_num); //peut-être conflit de type
	simptcp_set_head_len   (sockpdu->out_buffer, SIMPTCP_GHEADER_SIZE);  
	simptcp_set_win_size   (sockpdu->out_buffer, sockpdu->receiving_window_size);
	
	simptcp_set_total_len   (sockpdu->out_buffer,(u_int16_t)lenght_msg+SIMPTCP_GHEADER_SIZE);
	
	memcpy(sockpdu->out_buffer+SIMPTCP_GHEADER_SIZE,msg,lenght_msg);
	simptcp_add_checksum(sockpdu->out_buffer,(u_int16_t)lenght_msg+SIMPTCP_GHEADER_SIZE);
	simptcp_print_packet(sockpdu->out_buffer);
	if (sockpdu->out_len<=1500)
...
#endif
	lock_simptcp_socket(sock);
		memcpy(&(sock->remote_simptcp),addr,sizeof(struct sockaddr));
		memcpy(&(sock->remote_udp),addr,sizeof(struct sockaddr));
		unlock_simptcp_socket(sock);
		//sock->remote_udp.sin_port=simptcp_entity.local_udp.sin_port;
	if (makePDU(sock,NULL,SYN,0)==0){ //flag 0x01=SYN	
		libc_sendto(simptcp_entity.udp_fd, sock->out_buffer, simptcp_get_total_len(sock->out_buffer) ,0, (struct sockaddr*)&(sock->remote_udp), len);
		sock->socket_state = &(simptcp_entity.simptcp_socket_states->synsent);
		start_timer(sock,4000);
		}
	else {
		printf("Echec création PDU SYN\n");
		return -1;
	}
	//while( sock->socket_state!=&(simptcp_entity.simptcp_socket_states->established) && sock->simptcp_send_count<15);
	while(sock->simptcp_send_count<15);
...
{ 
  ...
  printf("function %s called\n", __func__);
	...
	sock->new_conn_req=malloc((sizeof(struct simtcp_socket *))*n);
...
//on considère une seule demande de connexion possible à la fois
struct simptcp_socket * tampon = sock->new_conn_req[0];
sock->new_conn_req[0]=NULL;
sock->pending_conn_req--;
if (makePDU(sock,NULL,SYNACK,0)==0){
	libc_sendto(simptcp_entity.udp_fd, tampon->out_buffer, simptcp_get_total_len(tampon->out_buffer) ,0, (struct sockaddr*)&(tampon->remote_udp), *len);
	lock_simptcp_socket(tampon);
	tampon->socket_state=&(simptcp_entity.simptcp_socket_states->synsent);
	start_timer(tampon,4000);
	
	while(tampon->socket_state!=&(simptcp_entity.simptcp_socket_states->synsent) && sock->simptcp_send_count<15);
	
	stop_timer(tampon);
	print_simptcp_socket(tampon);
	tampon->next_seq_num++;
	if (sock->socket_state!=&(simptcp_entity.simptcp_socket_states->synsent)) {
	return -1;	
	}
	else {
	return 0;
	}
unlock_simptcp_socket(tampon);	
}
...
if (simptcp_get_flags(buf)==SYN && simptcp_get_seq_num(buf)==sock->next_seq_num && sock->pending_conn_req!=sock->max_conn_req_backlog){
	struct simptcp_socket * insert_sock;
	int fd= create_simptcp_socket();
	insert_sock=simptcp_entity.simptcp_socket_descriptors[fd];	
	lock_simptcp_socket(insert_sock);	
	insert_sock->remote_udp=sock->remote_udp; // mm porte d'entrée	
	insert_sock->local_simptcp = sock->local_simptcp;	
	insert_sock->remote_simptcp.sin_family = AF_INET;	
	insert_sock->remote_simptcp.sin_addr.s_addr = sock->remote_udp.sin_addr.s_addr;		
	insert_sock->remote_simptcp.sin_port=sock->remote_simptcp.sin_port; 	
	insert_sock->next_ack_num = simptcp_get_seq_num(buf) + 1;	
	insert_sock->next_seq_num = simptcp_get_ack_num(buf);
	
	insert_sock->socket_type=nonlistening_server;
	insert_sock->socket_state=&(simptcp_entity.simptcp_socket_states->synrcvd);
	sock->pending_conn_req++;
	sock->new_conn_req[0]=insert_sock;
	
	unlock_simptcp_socket(insert_sock);
	}
...
if (simptcp_get_flags(buf)==SYNACK && simptcp_get_seq_num(buf)==sock->next_seq_num) {
	if (makePDU(sock,NULL,ACK,0)==0){
	libc_sendto(simptcp_entity.udp_fd, sock->out_buffer, simptcp_get_total_len(sock->out_buffer) ,0, (struct sockaddr*)&(sock->remote_udp), len);
	lock_simptcp_socket(sock);
	...
	sock->next_seq_num++;
	if (sock->socket_state!=&(simptcp_entity.simptcp_socket_states->established)) {
	printf("ERREUR SUR L'ETAT: PAS ESTABLISHED");	
	}
...
void synsent_simptcp_socket_state_handle_timeout (struct simptcp_socket* sock)
{
	...
	libc_sendto(simptcp_entity.udp_fd, sock->out_buffer, simptcp_get_total_len(sock->out_buffer) ,0, (struct sockaddr*)&(sock->remote_udp), sock->out_len);
	lock_simptcp_socket(sock);
	sock->simptcp_send_count++;
	unlock_simptcp_socket(sock);
